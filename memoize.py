...
class memoized_f(keyed_object):
    ...
    def get_cache_key(self, *args, **kwargs):
        ...
        cache_key = tuple([self.get_interval_key(self.f) + ]args_keys + kwargs_keys)
        return cache_key
...
class memoizing_dec(object):
    ...
    """
    def __init__(self, db, recalculate_determiner, cache_determiner):
        self.db, self.recalculate_determiner, self.cache_determiner = db, recalculate_determiner, cache_determiner

    def __call__(self, f):
        return memoized_f(self.db, self.recalculate_determiner, self.cache_determiner)
